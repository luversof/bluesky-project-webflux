...
//		return ReactiveSecurityContextHolder.getContext().flatMap(x -> {
//			Authentication authentication2 = x.getAuthentication();
//			return bookkeepingService.findByUserId(null).next();
//		});
		return ReactiveUserDetailsContextHolder.getContext().flatMap(x -> {
			UserDetails userDetails = x.getUserDetails();
			return bookkeepingService.findByUserId(null).next();
		});
	}
}
